...
Input dataframes expected:
  ...
  - basin_df  : site_id, basin_area_km2
...
def make_basin_df(sites):
    ...
    for site in sites:
        ...
        rows.append({
            "site_id": site,
            "basin_area_km2": data.basin_area
        })

    return pd.DataFrame(rows)


def make_site_df(sites):
    ...
    for site in sites:
        ...
        data = get_data(site)
        daily_avg_precip = (data.rain.groupby("date", as_index=False)["precip_in_1d"].mean())
        daily_avg_precip["site_id"] = site
        

        # Aggregate water levels to each calendar day
        water_max = aggregate_by_interval(site_uid=site, value_col="nitrate_con", interval="1D", agg_func="max").to_frame()
        water_max["date"] = water_max.index.date
        water_max = water_max.reset_index(drop=True).rename(columns={'nitrate_con':'nitrate_max'})

        water = aggregate_by_interval(site_uid=site, value_col="nitrate_con", interval="1D", agg_func="mean").to_frame()
        ...
        df = pd.merge(water, daily_avg_precip, on='date')
...
def build_daily_df(site_df, basin_df):
    """
    Aggregate site-level data to one row per date.

    - nitrate_con  : weighted mean by basin_area_km2 (larger basins = more load)
    ...
    # Merge basin area
    site_df = site_df.merge(basin_df[["site_id", "basin_area_km2"]], on="site_id")

    # Weighted mean helper
    def wmean(group, col):
        w = group["basin_area_km2"]
...
def run_ftest(doy_df, response_var, K=2, maxlags=14):
    ...
    harmonic_cols  = [f"sin{k}" for k in range(1, K+1)] + \
                     [f"cos{k}" for k in range(1, K+1)]
...
def print_results(res):
    ...
    rec  = ("standard F-test" if 1.7 <= dw <= 2.3
            else "HAC-robust F-test ← use this (autocorrelation present)")
    ...
    print(f"  HAC-robust F({2*res['K']}, {res['n']-2*res['K']-2}) = "
          f"{res['F_hac']:.3f},   p = {res['p_hac']:.4f}")
    print()
...
def plot_diagnostics(res_nitrate, res_violation, save_path="ftest_diagnostics.png"):
    ...
    def _plot_fit(ax, res, label, y_raw_col, y_label, color):
        ...
        ax.scatter(df["doy"], df[y_raw_col], s=8, alpha=0.4,
                   color="grey", label="Observed (DOY avg)")
        ax.plot(df["doy"], mod_r.fittedvalues, color="steelblue",
                lw=1.8, linestyle="--", label="Rainfall only (H₀)")
        ax.plot(df["doy"], mod_f.fittedvalues, color=color,
                lw=2.2, label=f"+ Seasonal (Hₐ), R²={res['R2_full']:.3f}")

        p_use = res["p_hac"] if res["durbin_watson"] < 1.7 or res["durbin_watson"] > 2.3 \
                else res["p_standard"]
        sig   = "***" if p_use < 0.001 else ("**" if p_use < 0.01
                                              else ("*" if p_use < 0.05 else "ns"))
    ...
    def _plot_resid(ax, res, label, color):
        ...
        ax.set_title(f"{label} — Full model residuals\n"
                     f"Durbin-Watson = {dw_val:.3f}", fontsize=10,
                     color=dw_col)
        ax.set_xlabel("Day of Year")
    ...
    _plot_fit(ax1, res_nitrate,   "Nitrate Concentration",
              "nitrate_con",    "mg/L (area-wtd mean)",  "darkorange")
    _plot_fit(ax2, res_violation, "Violation Rate (logit)",
              "violation_logit","logit(violation rate)", "mediumseagreen")
...
def run_pipeline(site_df, basin_df, K=2):
    """
    ...
    site_df  : DataFrame with columns [date, site_id, violation, nitrate_con,
                                        precip_in_1d]
    basin_df : DataFrame with columns [site_id, basin_area_km2]
    K        : Number of Fourier harmonics (start with 2, try 3 if residuals
               show remaining seasonal structure)
    """
    print("Step 1: Merging basin areas and aggregating to daily...")
    daily_df = build_daily_df(site_df, basin_df)
    ...
    print(f"  → {len(doy_df)} DOY rows, "
          f"{doy_df['n_years'].mean():.1f} avg years per DOY")
    ...
    plot_diagnostics(res_nitrate, res_violation,
                     save_path="ftest_diagnostics.png")
...
# ── Example call (replace with your actual dataframes) ────────────────────────
if __name__ == "__main__":
    # Replace these with your real dataframes:
    # results = run_pipeline(site_df, basin_df, K=2)

    # ── Minimal synthetic example to verify the pipeline runs ─────────────
    rng  = np.random.default_rng(42)
    n    = 2000
    dates = pd.date_range("2016-01-01", periods=n, freq="D")
    dates = np.random.choice(dates, size=n, replace=False)

    # site_df_ex = pd.DataFrame({
    #     "date":        dates,
    #     "site_id":     rng.choice(["S1","S2","S3","S4","S5"], size=n),
    #     "violation":   rng.binomial(1, 0.15, size=n),
    #     "nitrate_con": np.abs(rng.normal(5, 2, size=n)),
    #     "precip_in_1d":rng.exponential(0.05, size=n),
    # })

    # basin_df_ex = pd.DataFrame({
    #     "site_id":        ["S1","S2","S3","S4","S5"],
    #     "basin_area_km2": [120, 340, 85, 210, 500],
    # })

    site_df = make_site_df(get_site_ids())
    basin_df = make_basin_df(get_site_ids())
