...
- **Classification (violation flagging):** ROC-AUC 0.86, PR-AUC 0.70 against a 25.8% base rate (2.7× better than chance), and per-site AUC 0.90 — excellent at timing when a basin spikes. A Brier score of 0.129 (vs. a 0.191 base-rate floor, a 33% skill improvement) confirms well-calibrated probabilities. Tuned toward recall, it catches 89% of violations at a 55% false-discovery rate, with a knob to trade recall against precision.

- **Regression (concentration):** R² ≈ 0.38 (up from the 0.24 baseline), ~4.4 mg/L typical error. To account for the high variability in average site nitrate values we report two mean-normalized scores: within-basin R² (0.40) calculates R² after subtracting a site's mean from both the prediction and the actual, and between-basin R² (0.26) compares the predicted and actual site means. These scores therefore reflect that the regressor is good at detecting the direction of changes within a site, but bad at pinning the absolute level of an unseen basin. It's a "when will nitrate spike and roughly how bad will it be" tool.

These numbers reflect the full models. The deployed widget runs reduced `light_CLF` and `light_REG` models, restricted to the feature set a browser can assemble; they score marginally lower (PR-AUC 0.689, R² 0.371) and catch 88% of violations at a 53% false-discovery rate.
