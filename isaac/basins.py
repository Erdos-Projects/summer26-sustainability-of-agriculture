...
BasinCache(path)            persistent (lat, lon)-keyed cache for quick re-display
...
def render_basin_map(
    ...
    *,
    ...
    """Render one or more basins on an interactive map.

    Parameters
    ----------
    basins : GeoDataFrame
        Output of ``delineate_basin`` / ``delineate_basins``. Needs a ``site_id``
        column; ``site_lat`` / ``site_lon`` are used for the pour-point markers.
    color : str or None
        How to color the basins:
          * ``"site_id"`` (default) -- one discrete color per site + legend. Best for
            a handful of basins; colors recycle past ~20 sites.
          * a numeric column, e.g. ``"area_km2"`` -- continuous colorscale + colorbar.
          * ``None`` -- a single uniform color, no legend. Best for showing many
            basins at once (coverage view).
    map_style : str
    ...
    # Treat a non-numeric color column as categorical (discrete colors + legend).
    if color is not None and color in basins.columns and not pd.api.types.is_numeric_dtype(basins[color]):
        basins[color] = basins[color].astype(str)
    ...
    fig = px.choropleth_map(
        ...
        zoom=zoom,
        map_style=map_style,
        hover_name="site_id" if "site_id" in basins.columns else None,
        hover_data=hover_data,
    )

    if show_points and {"site_lat", "site_lon"}.issubset(basins.columns):
        pts = basins.drop_duplicates("site_id") if "site_id" in basins.columns else basins
        fig.add_trace(
            go.Scattermap(
                lat=pts["site_lat"],
                lon=pts["site_lon"],
...
# --------------------------------------------------------------------------- #
# Coordinate-keyed cache                                                       #
# --------------------------------------------------------------------------- #
class BasinCache:
    """A persistent, coordinate-keyed store of delineated basins, backed by GeoParquet.

    Each basin is keyed by its (lat, lon) rounded to ``precision`` decimals (5 ~ 1 m),
    so re-requesting a point you've already delineated reads from disk instead of
    calling the NLDI. One file, loaded into memory on init; ``ensure`` only hits the
    API for points that aren't cached yet.

    Usage
    -----
    >>> cache = BasinCache("basins_cache.parquet")
    >>> subset = cache.ensure(pairs)        # delineates only the new ones, saves, returns all requested
    >>> cache.render(pairs).show()          # display just those
    >>> cache.render().show()               # display everything cached
    >>> one = cache.get(41.587, -93.625)    # one-row GeoDataFrame, or None

    Single-process / last-write-wins -- fine for a research cache, not for concurrent writers.
    """

    _COLS = ["_key", "site_id", "comid", "site_lat", "site_lon", "area_km2"]

    def __init__(self, path, precision: int = 5):
        self.path = Path(path)
        self.precision = int(precision)
        self.gdf = self._load()

    # -- keys -------------------------------------------------------------- #
    def _key(self, lat, lon) -> str:
        p = self.precision
        return f"{round(float(lat), p):.{p}f},{round(float(lon), p):.{p}f}"

    def _keys_for(self, gdf) -> list:
        return [self._key(la, lo) for la, lo in zip(gdf["site_lat"], gdf["site_lon"])]

    # -- persistence ------------------------------------------------------- #
    def _load(self) -> gpd.GeoDataFrame:
        if self.path.exists():
            g = gpd.read_parquet(self.path)
            if "_key" not in g.columns:
                g["_key"] = self._keys_for(g)
            return g
        return gpd.GeoDataFrame({c: [] for c in self._COLS}, geometry=[], crs="EPSG:4326")

    def save(self) -> Path:
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.gdf.to_parquet(self.path)
        return self.path
    ...
    def __contains__(self, latlon):
        lat, lon = latlon
        return self._key(lat, lon) in set(self.gdf["_key"])

    # -- read -------------------------------------------------------------- #
    def get(self, lat, lon):
        """Return the cached one-row GeoDataFrame for a point, or None if absent."""
        hit = self.gdf[self.gdf["_key"] == self._key(lat, lon)]
        return hit.reset_index(drop=True) if len(hit) else None

    def _subset(self, sites, lat_col, lon_col, id_col) -> gpd.GeoDataFrame:
        """Rows for the requested points, in request order, de-duplicated."""
        wanted, seen = [], set()
        for _, la, lo in _iter_sites(sites, lat_col, lon_col, id_col):
            k = self._key(la, lo)
            if k not in seen:
                seen.add(k)
                wanted.append(k)
        by_key = self.gdf.drop_duplicates("_key").set_index("_key")
        present = [k for k in wanted if k in by_key.index]
        out = by_key.loc[present].reset_index()
        return gpd.GeoDataFrame(out, geometry="geometry", crs="EPSG:4326")

    # -- write ------------------------------------------------------------- #
    def ensure(self, sites, *, lat_col="lat", lon_col="lon", id_col=None,
               simplified=True, compute_area=True, save=True) -> gpd.GeoDataFrame:
        """Delineate only the points not already cached, append + persist them, and
        return the cache subset for *all* requested points (cached + newly added)."""
        triples = list(_iter_sites(sites, lat_col, lon_col, id_col))
        have = set(self.gdf["_key"])
        missing = [(sid, la, lo) for sid, la, lo in triples if self._key(la, lo) not in have]

        if missing:
            new = delineate_basins(
                [(sid, la, lo) for sid, la, lo in missing],
                simplified=simplified, compute_area=compute_area,
            )
            new["_key"] = self._keys_for(new)  # recompute from stored coords (robust to skips)
            self.gdf = gpd.GeoDataFrame(
                pd.concat([self.gdf, new], ignore_index=True), crs="EPSG:4326"
            )
            if save:
                self.save()

        return self._subset(sites, lat_col, lon_col, id_col)

    # -- display ----------------------------------------------------------- #
    def render(self, sites=None, *, lat_col="lat", lon_col="lon", id_col=None, **map_kwargs):
        """Render the whole cache (``sites=None``) or just the requested subset.

        If ``sites`` is given, any not-yet-cached points are delineated and added first.
        Extra kwargs pass through to ``render_basin_map`` (color, map_style, opacity, ...).
        """
        gdf = self.gdf if sites is None else self.ensure(sites, lat_col=lat_col, lon_col=lon_col, id_col=id_col)
        if gdf.empty:
            raise ValueError("Nothing to render -- the cache is empty.")
        return render_basin_map(gdf.drop(columns="_key", errors="ignore"), **map_kwargs)
