...
def extend_years_forward(annual, last_year):
    """Broadcast an annual frame's final-year values forward through `last_year` (ffill over a
    reindexed contiguous year range). A no-op for years already present."""
...
def make_recipe(extend):
    """base covariates (target = 'violation'); if `extend`, surplus broadcast forward past 2017."""
