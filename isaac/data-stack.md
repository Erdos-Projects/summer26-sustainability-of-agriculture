...
- **IOWA WQIS** (https://iwqis.iowawis.org/)
  ...
  - Access by request for the Iowa sensors. USDA sensors can be accessed more easily. (I'm in email correspondence with the admins.)
...
- **SSURGO Soil Data**
  ...
  - See also the notebooks `SSURGO_data_imports_experiments.ipynb` and `more_SSURGO.ipynb`.
...
- **All other datasets with checks above:**
  Check out the `test-data-access.ipynb` notebook. Note the OpenET example requires an access code. I put mine in the notebook (commented out) but there are query limits so please don't run it a bunch.