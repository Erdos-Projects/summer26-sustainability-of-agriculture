...
def build(name, recipe, target_col, task):
    ...
    scores = compare_many(
        {name: recipe}, sites=None, target_col=target_col, task=task, extra_importance_test=True, **REAL_XGB
    )
    ...
    model, feat, _imp = fit_full(recipe, sites=None, target_col=target_col, task=task, final_fit=True, progress=True, **REAL_XGB)
    for f in save_model(model, feat, str(OUT / f"{name}.json"), task=task, target_col=target_col):
        print(f"  wrote {f}")


def main():
    build("recipe_REG1", recipe_REG, target_col="nitrate_con", task="reg")
    build("recipe_CLF1", recipe_CLF, target_col="violation", task="clf")
