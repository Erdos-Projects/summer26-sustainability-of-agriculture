"""Build a virtual SiteData for an arbitrary (lat, lon) in Iowa (re-grained).

Now trivial: fetch the NLDI basin for the pin, then access.build_virtual_site_data assembles the
SiteData via build_site_view over grid_global + the global-table joins. The old per-pin work
(build_grid_from_basin's local Voronoi + crops_grid/surplus_grid/weather_for_grid aggregation)
collapses into a grid intersection + joins -- no raster/overlay per pin. That is what removes the
virtual-site latency for the widget forecast.

deploy/ is an APP: it imports src/data (never src/build). The NLDI call is the only network step.
"""
...
def delineate_basin_for_pin(lat: float, lon: float, timeout: int = 60) -> gpd.GeoDataFrame:
    """Upstream NLDI basin polygon for an arbitrary (lat, lon). Two requests: resolve the pin to
    the enclosing NHDPlus COMID, then fetch its upstream basin. Raises ValueError off-network."""
...
def build_virtual_basin(lat: float, lon: float, target_year: int = TARGET_YEAR, timeout: int = 60) -> SiteData:
    ...
    Weather spans target_year ±2 months so trailing rolling/lag features have lead-in. Everything
    else is a grid_global intersection + joins (see access.build_virtual_site_data).
    """
    ...
    return access.build_virtual_site_data(
        basin, lat, lon, weather_start=ystart - _BUFFER, weather_end=yend + _BUFFER
    )
