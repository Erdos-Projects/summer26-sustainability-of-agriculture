...
- **`check_forecast` measures implementation, not geometry.** Both sides get the same cached NLDI basin at the same outlet and neither snaps, so what it reports is the browser-vs-Python difference alone: currently ~0.012 mg/L mean against the model's own 4.2 mg/L LOFO RMSE. It does NOT cover snapping — that is `_make_basins.snap_comid` vs `forecast.js::snapComid`, and letting it run inside this harness only ever meant each side scoring a different catchment.
