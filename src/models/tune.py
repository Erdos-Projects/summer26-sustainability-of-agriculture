...
_LEAD_COLS = ("recipe", "task", "metric", "lofo", "loso", "gap", "best_iteration")
...
def tune_task(task, recipe, target_col, metric, sites, depths, lrs, regular, n_splits, ceiling):
    ...
    # tree count for the winning config: refit its grouped CV once and take the median best_iteration
    # (+1 -> tree count) across folds. This is the "best_iteration from a tuning run" that the full
    # train (build_model) reads to fit the deployable model on 100% of rows with early stopping off.
    rank_grp = fam_grp if n_fam >= 2 else site_grp
    fold_iters = [int(m.best_iteration) + 1 for _te, m in _grouped_models(X, y, rank_grp, task, n_splits, **winner)]
    best_iter = int(pd.Series(fold_iters).median()) if fold_iters else None
    ...
    summary = {
        ...
        "best_iteration": best_iter,  # median fold tree count -> full train's n_estimators
        **winner,
    }
    ...
    print(f"  best_iteration (median across {rank_col.upper()} folds): {best_iter} "
          f"-> full train fits n_estimators={best_iter}, early_stopping_rounds=None on all rows")
...
def main():
    ...
    ap.add_argument("--module", default="src.features.recipes",
                    help="module to import recipe_REG/recipe_CLF from (default: src.features.recipes)")
