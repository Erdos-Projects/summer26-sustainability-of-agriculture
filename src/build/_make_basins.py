...
# Only these columns are ever read (snap_comid: COMID/TotDASqKM/GNIS_NAME; _nearby_river: the same
# two). The saved layer carries ~130 NHDPlus attribute columns over 300k+ rows, so projecting at
# read time keeps this from ballooning to a few hundred MB for four fields.
_FLOWLINE_COLS = ["COMID", "GNIS_NAME", "TotDASqKM", "StreamOrde", "geometry"]
...
def _load_flowlines() -> gpd.GeoDataFrame | None:
    ...
    layer covers Iowa; pynhd's bygeom returns INTERSECTING features, so reaches crossing the state
    line come through whole. To generalize, re-run that builder over a wider geometry. A site
    outside the cached extent finds no flowlines and flag_river comes out False -- a coverage gap
    degrades to a warning HERE, but note basin1 RAISES rather than silently degrading.
    """
    if not _FLOWLINES_PATH.exists():
        ...
        return None
    try:
        fl = gpd.read_parquet(_FLOWLINES_PATH, columns=_FLOWLINE_COLS)
    except (ValueError, KeyError):  # older layer missing one of the optional columns
...
def main(api_keys=None, force=False, usgs_only=False, wqs_only=False, recalculate=False, report_auto=False) -> None:
    ...
    # NOTE: the column is `draining_area` in site_location_metadata.csv (the IWQIS registry's
    # spelling, carried through site_locations.py). It is looked up by name, and .get() would
    # silently yield an all-None mapping if it were wrong -- disabling the refinement without a
    # single error -- so fail loudly instead if it ever goes missing.
    KM2_PER_SQMI = 2.58999
    if "draining_area" not in meta.columns:
        raise KeyError(
            "site_location_metadata.csv has no 'draining_area' column -- the basin1 snap's "
            "area-matching refinement would be silently disabled. Rebuild the water metadata."
        )
    # A reported area of 0 or 1 is the IWQIS registry's "unknown" sentinel, not a 1 sq mi basin --
    # feeding it to the snap would drag the sensor onto the smallest reach in range.
    reported_area = {
        uid: (float(a) * KM2_PER_SQMI if pd.notna(a) and float(a) > 1.0 else None)
        for uid, a in meta.set_index("site_uid")["draining_area"].items()
    }
