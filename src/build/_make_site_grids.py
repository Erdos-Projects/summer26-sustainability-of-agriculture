...
Read back by access.get_grid, which falls back to computing live when a file is missing or stale -- so this step is an optimization, never a prerequisite. Virtual/ungauged sites (deploy/build_virtual_basin.py, and the static-site reach pass) delineate a basin from an arbitrary pin and can never be precomputed; they keep calling site_view.build_site_view directly and are unaffected.

Staleness is by mtime against both inputs: the site's basin parquet (the widget can rewrite basin4 in place under an unchanged name) and grid_global.parquet (a grown covariate bbox renumbers global_node_id, invalidating every persisted grid at once).

    python -m src.build._make_site_grids                 # every stale site
    python -m src.build._make_site_grids --force         # rebuild all
    python -m src.build._make_site_grids --site WQS0003  # one site (repeatable)
"""
...
def _sites() -> list[str]:
    """Every site with a preferred basin -- the same cohort get_grid can be asked about."""
    return get_basin_metadata()["site_uid"].tolist()
...
def build_one(site_uid: str) -> int:
    ...
    if grid.empty:
        # No grid_global cell intersects the basin. An empty frame round-trips with object dtypes
        # instead of the real float64/int64 schema, so persist nothing and let get_grid rebuild live.
        return 0
